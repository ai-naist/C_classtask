...
/*ee2021
//  コード内のすべての"ee2021"について
//  ee2021  --->>>  */
//  と変更すると、gnuplotによる描画を有効にできる
//
//  gnuplot
...
inline void nearest_TSP(/*ee2021 FILE **, /**/ const int, const int, const CityInfo_t *, int *, int *, double *);
...
inline void plot_opt(const int, const int, FILE **, const CityInfo_t *, const int *);
void plot_end(FILE **);
/**/
/*
...
int main(void)
{
    ...
    */
    ...
    get_array(zz, int, N + 1); // メモリの確保と初期化
    // §3   -----おわり-----
    ...
    */
    ...
    nearest_TSP(/*ee2021 &gp, /**/ N, start, cityinfodata, z, zz, &totaldistance);
    // §4   -----おわり-----
    ...
    */
    int *zz_old;
    int *zz_new;
    get_array(zz_old, int, N + 1);
    get_array(zz_new, int, N + 1);
    ...
    do
    {
        check = false; // チェックの初期化
        opt2_TSP(&check, N, &tdis_new, cityinfodata, zz_old, zz_new);
        /*ee2021
        plot_opt(start, N, &gp, cityinfodata, zz_new);
        /**/
    } while (check); // 改善のチェック
...
inline void nearest_TSP(/*ee2021 FILE **pgp, /**/ const int N, const int start, const CityInfo_t *data, int *z, int *zz, double *ptdis)
{
    // fp3を追加書き込みモードでオープン
    if (fopen_s(&fp3, "Inoue_Akihiro_task8_bonus2.csv", "a") != 0)
        exit(EXIT_FAILURE); // エラー処理
...
/*ee2021
// gnuplot に描画するための設定
void plot_set(const int start, FILE **pgp, const double a, const double b, const int N, const CityInfo_t *data, const int *zz)
{

    fprintf(*pgp, "set multiplot\n"); // マルチプロットモード

    // 範囲の指定
    fprintf(*pgp, "set xrange [%lf:%lf]\n", a, b);
    fprintf(*pgp, "set yrange [%lf:%lf]\n", a, b);
    fprintf(*pgp, "unset key\n");  // 凡例を非表示
    fprintf(*pgp, "set notics\n"); // 目盛りを非表示
    ...
    fprintf(*pgp, "plot 'point0.dat' with points pointtype 7 pointsize 1.5 linetype rgbcolor 'red'\n");
    fclose(fp_p);
    fflush(*pgp);
    Sleep(3000);
    ...
    fprintf(*pgp, "plot 'point.dat' with points pointtype 6 pointsize 1.5 linetype rgbcolor 'black'\n");
    fclose(fp_p);
    fflush(*pgp);
}

// 都市間を線分で結ぶ
inline void plot_line(const int start, const int k, FILE **pgp, const CityInfo_t *data, const int *zz)
{
    FILE *fp_l;
    if (fopen_s(&fp_l, "line.dat", "w") != 0)
        exit(EXIT_FAILURE);
    ...
    // 線分のプロット
    fprintf(*pgp, "plot 'line.dat' with lines linetype rgbcolor 'web-green' linewidth 2\n");
    fclose(fp_l);
    fflush(*pgp);
    Sleep(30 + k / 10);
}
...
// 2opt法の結果を描画する
inline void plot_opt(const int start, const int N, FILE **pgp, const CityInfo_t *data, const int *zz)
{
    fprintf(*pgp, "set multiplot\n");

    FILE *fp_opt;
    if (fopen_s(&fp_opt, "optplot.dat", "w") != 0)
        exit(EXIT_FAILURE);

    for (int m = 1; m <= N + 1; m++)
    {
        fprintf(fp_opt, "%lf %lf\n", data[zz[m]].x, data[zz[m]].y);
    }

    fprintf(*pgp, "plot 'point0.dat' with points pointtype 7 pointsize 1.5 linetype rgbcolor 'red'\n");
    fprintf(*pgp, "plot 'point.dat' with points pointtype 6 pointsize 1.5 linetype rgbcolor 'black'\n");
    fflush(*pgp);
    fprintf(*pgp, "plot 'optplot.dat' with lines linetype rgbcolor 'web-green' linewidth 2\n");
    fclose(fp_opt);
    fflush(*pgp);
    Sleep(1000);
}
...
// gnuplot の終了処理
void plot_end(FILE **pgp)
{
    system("pause");
    fprintf(*pgp, "set nomultiplot\n");
    fprintf(*pgp, "exit\n"); // gnuplotの終了
    fflush(*pgp);            // バッファのデータを吐き出す
    _pclose(*pgp);
}
/**/