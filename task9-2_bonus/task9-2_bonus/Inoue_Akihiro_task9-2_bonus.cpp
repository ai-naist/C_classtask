...
void OutputData(const int, const int, const double, const double, const double, const double,
                double *, double *, double *, double *, double *, double *, double *);
void OutputFreq(const int, const int, const double, const double *, const double *, const double *, int *, int *, double *, double *);
void CreateRN(const int, double *, double *, double *, double *, double *, int *);
...
int main(void)
{
    ...
    // fp オープン aモード
    if (fopen_s(&fp, "Inoue_Akihiro_task9-2_bonus.csv", "a") != 0)
    ...
    // 乱数の型を選択して生成
    CreateRN(N, &a, &b, &mu, &sigma, x, &choice);
    ...
    OutputData(N, choice, a, b, mu, sigma, &xmax, &xmin, &sum, &avx, &vx, &sx, x);
...
void OutputData(const int N, const int choice, const double a, const double b, const double mu, const double sigma,
...
// 乱数の型を選択して生成
void CreateRN(const int N, double *p_a, double *p_b, double *p_mu, double *p_sigma, double *x, int *pchoice)
{
    bool check = true;
    do
    {
